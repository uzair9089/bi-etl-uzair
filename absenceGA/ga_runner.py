    ...
    #for merchant in param.merchant_list:
      ...
      if traffic_results.get('rows', []):
        for row in traffic_results.get('rows'):
          try:
            cursor.execute("""INSERT INTO ga.traffic_ab (year_month, source, page_path, host, session_count, device_category, user_count, sessions, visit_count, newuser_count, start_date) 
